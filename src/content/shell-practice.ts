...
export const shellPractice: Record<string, Record<string, Section[]>> = {
  terminal: {
    'terminal-vs-shell': [
      {
        type: 'shell',
        content: 'Get your bearings. Print the working directory, list what is in it, read notes.txt, then move into the lessons directory and list it too.',
        shell: {
          title: 'First steps',
          ...
          checks: [
            ...
            { name: 'Read notes.txt with cat', check: s => ran(s, /^\s*cat\s+.*notes\.txt/) || 'run cat notes.txt' },
    ...
    'stdin-stdout-pipes': [
      {
        type: 'shell',
        content: 'Redirect and pipe. Save the output of ls -l to files.txt, append the line "end of listing" to it, then use a pipe to count how many lines in data/songs.txt mention Track 3.',
        shell: {
          title: 'Redirection and a pipe',
          hints: ['> overwrites, >> appends: ls -l > files.txt then echo "end of listing" >> files.txt', 'grep "Track 3" data/songs.txt | wc -l counts matching lines (grep -c does it too).'],
          solution: 'ls -l > files.txt\necho "end of listing" >> files.txt\ngrep "Track 3" data/songs.txt | wc -l',
          checks: [
            ...
            { name: 'Counted the Track 3 lines through a pipe', check: (s, out) => (ran(s, /grep.*\|.*wc/) || ran(s, /grep\s+-c/)) && /\b2\b/.test(out) || 'grep the file and pipe into wc -l; the answer should be 2' },
          ],
        },
      },
      {
        type: 'shell',
        content: 'Build a small pipeline: print the names from data/team.csv (the first field, skipping the header line), sorted alphabetically, and save the result to names.txt.',
        shell: {
          title: 'A pipeline that transforms data',
          hints: ['tail -n 4 data/team.csv skips the header (there are 5 lines).', 'cut -d , -f 1 takes the first comma-separated field; then sort; then > names.txt.'],
          solution: 'tail -n 4 data/team.csv | cut -d , -f 1 | sort > names.txt\ncat names.txt',
          checks: [
            { name: 'names.txt has the four names sorted', check: s => { const t = fileText(s, 'names.txt'); return (t !== null && t.trim().split('\n').join(',') === 'Ada,Leo,Maya,Sam') || `names.txt should contain Ada, Leo, Maya, Sam in that order` } },
  ...
  bash: {
    'first-script': [
      {
        type: 'shell',
        content: 'Scripts are just files of commands. Run the existing scripts/hello.sh, then create your own greet.sh in the scripts directory that echoes "Hello from greet", make it executable, and run it.',
        shell: {
          title: 'Write and run a script',
          hints: ['./scripts/hello.sh runs a script by path.', 'echo \'echo "Hello from greet"\' > scripts/greet.sh writes a one-line script; chmod +x scripts/greet.sh makes it runnable.'],
          solution: './scripts/hello.sh\necho \'echo "Hello from greet"\' > scripts/greet.sh\nchmod +x scripts/greet.sh\n./scripts/greet.sh',
          checks: [
            { name: 'Ran hello.sh', check: s => ran(s, /scripts\/hello\.sh/) || 'run ./scripts/hello.sh' },
            { name: 'scripts/greet.sh exists and echoes the greeting', check: s => { const t = fileText(s, 'scripts/greet.sh'); return (t !== null && /echo.*Hello from greet/.test(t)) || 'create scripts/greet.sh containing echo "Hello from greet"' } },
            ...
            { name: 'Ran greet.sh and saw the greeting', check: (s, out) => ran(s, /greet\.sh\s*$/) && /Hello from greet/.test(out) || 'run ./scripts/greet.sh' },
          ],
        },
      },
    ],
    'files': [
      {
        type: 'shell',
        content: 'Organise a project. Create a directory project with src and docs inside it, move README.md into docs, copy notes.txt into src as tips.txt, and remove the lessons/day3.txt file.',
        shell: {
          title: 'Make, move, copy, remove',
          hints: ['mkdir -p project/src project/docs creates everything in one go.', 'mv README.md project/docs/ moves; cp notes.txt project/src/tips.txt copies with a new name; rm lessons/day3.txt deletes.'],
          solution: 'mkdir -p project/src project/docs\nmv README.md project/docs/\ncp notes.txt project/src/tips.txt\nrm lessons/day3.txt\ntree project',
          checks: [
            ...
            { name: 'notes.txt copied to project/src/tips.txt (original kept)', check: s => { const a = fileText(s, 'notes.txt'), b = fileText(s, 'project/src/tips.txt'); return (a !== null && b !== null && a === b) || 'cp notes.txt project/src/tips.txt' } },
    ...
    'pipes-redirection': [
      {
        type: 'shell',
        content: 'From data/songs.txt produce a de-duplicated, sorted list of the track numbers only and save it to numbers.txt. Then capture the error message from reading a file that does not exist into errors.log without it appearing on screen.',
        shell: {
          title: 'Sort, uniq and stderr',
          hints: ['cut -d " " -f 2 data/songs.txt gives the numbers; sort -n | uniq de-duplicates numerically.', 'cat nothing.txt 2> errors.log sends only stderr to the file.'],
          solution: 'cut -d " " -f 2 data/songs.txt | sort -n | uniq > numbers.txt\ncat numbers.txt\ncat nothing.txt 2> errors.log\ncat errors.log',
          checks: [
            { name: 'numbers.txt holds 3, 7, 12, 20', check: s => { const t = fileText(s, 'numbers.txt'); return (t !== null && t.trim().split('\n').join(',') === '3,7,12,20') || 'numbers.txt should be 3, 7, 12, 20 one per line' } },
