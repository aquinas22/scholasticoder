...
export default function sitemap(): MetadataRoute.Sitemap {
  const now = new Date()
  const top: MetadataRoute.Sitemap = ['', '/languages/', '/dojo/', '/challenges/', '/glossary/', '/progress/', '/editors/'].map(p => ({ url: `${BASE}${p}`, lastModified: now, changeFrequency: 'weekly', priority: p === '' ? 1 : 0.8 }))
