...
Free, hands-on coding lessons that run in your browser: 32 courses across programming languages, the web, tools and computer science, with an emphasis on Python. Live at **https://aquinas22.github.io/scholasticoder/**.

## What is inside

- **One simple lesson pattern.** Every lesson reads the same way: a short explanation with runnable examples, a common question answered plainly (with the usual mix-ups explained), checked exercises, and a quick quiz. 61 common questions across 22 courses are indexed at `/questions`. See `/method`.
- **Clickable key words.** Terms in lesson prose link to the glossary: click one for a definition, an example and the course that teaches it (`src/lib/terms.ts`, `src/components/Term.tsx`).
...
- **The Playground** (`/dojo`): a Python, JavaScript, TypeScript and shell playground with example programs, program input for `input()`, a virtual file system, on-demand scientific packages, and shareable links.
- **Python challenges** (`/challenges`): 34 checked problems from FizzBuzz to Dijkstra, in four levels (Basics, Core, Intermediate, Advanced).
- **Progress** (`/progress`) is stored in the browser (localStorage). No accounts, no tracking. A **glossary** (`/glossary`) links every term to the course that teaches it.
- **Themes.** Light and Dark site themes (Automatic follows the device setting), plus editor classics: Gruvbox, Tokyo Night, Catppuccin, Nord, Dracula, Solarized, One Dark, Rosé Pine, Monokai, GitHub. Defined in `src/lib/themes.ts`; every palette recolours the code editors too.
...
| Path | Purpose |
| --- | --- |
| `src/content/languages/*.ts` | One file per course: setup, lessons, sections |
...
| `src/content/challenges.ts` | The Python challenges |
| `src/content/glossary.ts` | Glossary entries, with examples and the words that link to them in prose |
| `src/content/quaestiones*.ts` | Common questions merged into lessons (Python, web, tools, systems). The file and type names are historical. |
| `src/lib/sql-seed.ts` | The sample database every SQL block queries |
| `src/lib/shell-sim.ts` | The simulated bash shell used by shell exercises and the Playground |
| `src/lib/themes.ts` | Colour palettes |
| `src/content/cheatsheets.ts`, `field-guides.ts` | Per-course reference material |
