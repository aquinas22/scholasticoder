...
export const pythonExtraLessons: Lesson[] = [
  {
    ...
    sections: [
      ...
      { type: 'code', language: 'python', content: `s = "Hello and welcome"\nprint(s[0], s[-1])        # H e\nprint(s[6:9])             # and\nprint(s[::-1])            # reversed\nprint(len(s), "welcome" in s)\n\nfor word in s.split():\n    print(word.upper(), end=" ")\nprint()` },
      ...
      { type: 'code', language: 'python', content: `raw = "   Sam Carter  \\n"\nname = raw.strip()\nprint(repr(name))\n\ncsv_line = "ada,36,london"\nfields = csv_line.split(",")\nprint(fields)\nprint(" | ".join(fields))\n\nprint("programming".replace("gram", "GRAM"))\nprint("notes.txt".endswith(".txt"), "notes.txt".removesuffix(".txt"))\nprint("42".isdigit(), "forty".isalpha(), "Hello World".istitle())\nprint("apple".find("p"), "apple".find("z"))   # 1 -1` },
      ...
      { type: 'code', language: 'python', content: `text = "Crème brûlée — ☕"\ndata = text.encode("utf-8")\nprint(type(data), len(text), len(data))\nprint(data[:6])\nprint(data.decode("utf-8") == text)\n\nprint(ord("☕"), chr(9749))` },
      {
        type: 'exercise',
        content: 'Write normalise_name(raw) that trims whitespace, collapses internal runs of spaces to one, and title-cases the result. "  maya   CHEN " should become "Maya Chen".',
        exercise: {
          title: 'Clean a name',
          starter: `def normalise_name(raw):\n    ...\n\n\nprint(normalise_name("  maya   CHEN "))\n`,
          ...
          tests: [
            { name: 'Trims, collapses, title-cases', check: `assert normalise_name("  maya   CHEN ") == "Maya Chen"` },
            { name: 'Already clean stays clean', check: `assert normalise_name("Anne of Green Gables") == "Anne Of Green Gables"` },
            { name: 'Tabs and newlines count as whitespace', check: `assert normalise_name("\\tsam\\n carter") == "Sam Carter"` },
          ],
        },
      },
      {
      ...
      {
        type: 'quiz',
        content: 'What does "apple"[1:-1] evaluate to?',
        quiz: {
          choices: [
            { text: '"ppl"', correct: true, explanation: 'Start at index 1 (p), stop before the last character (e). Slices exclude the stop index.' },
            { text: '"pple"', explanation: 'The -1 stop excludes the final character.' },
...

  {
    ...
    sections: [
      ...
      { type: 'code', language: 'python', content: `import re\n\nprint(re.sub(r"\\s+", " ", "too   many    spaces"))\nprint(re.sub(r"(\\w+)@(\\w+)", r"\\2 at \\1", "sam@home"))\nprint(re.sub(r"\\d+", lambda m: str(int(m.group()) * 2), "3 apples, 5 pears"))\nprint(re.split(r"[,;]\\s*", "red, green;blue,  yellow"))` },
      { type: 'warning', content: 'Regex is greedy by default: .* grabs as much as it can. Use .*? for the shortest match. And do not parse HTML or nested structures with regex; use a real parser for those.' },
      {
        ...
        exercise: {
          ...
          starter: `import re\n\n\ndef extract_dates(text):\n    ...\n\n\nprint(extract_dates("Opened 1998-03-21, renovated 2015-10-14, reopened 2026-09-05."))\n`,
          ...
          tests: [
            { name: 'Finds three dates', check: `assert extract_dates("Opened 1998-03-21, renovated 2015-10-14, reopened 2026-09-05.") == ["1998-03-21", "2015-10-14", "2026-09-05"]` },
      ...
      },
      {
        ...
        exercise: {
          ...
          tests: [
            { name: 'Redacts both emails', check: `assert redact("mail sam@example.com or maya@shop.co.uk") == "mail [redacted] or [redacted]"` },
...

  {
    ...
    intro: 'JSON is the common format of APIs, config files and data exports. Python\'s json module turns it into dicts and lists and back again in one call each.',
    sections: [
      ...
      { type: 'code', language: 'python', content: `import json\n\ntext = '{"name": "Maya", "age": 28, "hobbies": ["climbing", "chess"], "member": true, "manager": null}'\nrecord = json.loads(text)\nprint(type(record), record["hobbies"][0], record["member"], record["manager"])\n\nprint(json.dumps(record))\nprint(json.dumps(record, indent=2, sort_keys=True))` },
      ...
      { type: 'code', language: 'python', content: `import json\nfrom datetime import date\nfrom dataclasses import dataclass, asdict\n\n@dataclass\nclass Event:\n    name: str\n    day: date\n\ndef to_json(obj):\n    if isinstance(obj, date):\n        return obj.isoformat()\n    raise TypeError(f"cannot serialise {type(obj).__name__}")\n\nevents = [Event("Team lunch", date(2026, 7, 11)), Event("Product launch", date(2026, 11, 1))]\ntext = json.dumps([asdict(e) for e in events], default=to_json)\nprint(text)\n\nback = [Event(d["name"], date.fromisoformat(d["day"])) for d in json.loads(text)]\nprint(back[0].day.strftime("%d %B"))` },
      { type: 'note', content: 'JSON has no comments, no trailing commas and only double quotes. If json.loads complains about a config file a human wrote, one of those three is usually the reason.' },
      {
        ...
        exercise: {
          ...
          starter: `import json\n\nTEXT = '{"shelves": [{"topic": "fiction", "books": [{"title": "A", "pages": 400}]}, {"topic": "science", "books": [{"title": "B", "pages": 120}, {"title": "C", "pages": 60}]}]}'\n\n\ndef total_pages(text):\n    ...\n\n\nprint(total_pages(TEXT))\n`,
    ...
    sections: [
      ...
      { type: 'tip', content: 'groupby only groups adjacent items, so sort by the key first. If you want unordered grouping, reach for defaultdict(list) instead.' },
      {
        ...
        exercise: {
          ...
          tests: [
            { name: 'Groups by length', check: `assert group_by_length(["red", "to", "yellow", "blue", "sky"]) == {3: ["red", "sky"], 2: ["to"], 6: ["yellow"], 4: ["blue"]}` },
            { name: 'Returns a plain dict', check: `assert type(group_by_length(["a"])) is dict` },
            { name: 'Top letters', check: `assert top_letters("Hello, balloon!", 2) == [("l", 4), ("o", 3)]` },
...

  {
    ...
    sections: [
      ...
      { type: 'code', language: 'python', content: `tree = {\n    "name": "house",\n    "children": [\n        {"name": "downstairs", "children": [{"name": "kitchen", "children": []}, {"name": "living room", "children": []}]},\n        {"name": "upstairs", "children": [{"name": "bedroom", "children": []}]},\n    ],\n}\n\ndef show(node, depth=0):\n    print("  " * depth + node["name"])\n    for child in node["children"]:\n        show(child, depth + 1)\n\ndef count_nodes(node):\n    return 1 + sum(count_nodes(c) for c in node["children"])\n\nshow(tree)\nprint(count_nodes(tree), "rooms")` },
